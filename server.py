...
class ProxyHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            target_url = data.get('apiUrl', '').strip() or os.environ.get('AI_API_URL', '').strip()
            api_key = os.environ.get('AI_API_KEY', '').strip()
            if not target_url:
                self._send_json(400, {'error': '缺少 apiUrl，请在后端配置 AI_API_URL 环境变量'})
                return
            if not api_key:
                self._send_json(500, {'error': '后端未配置 AI_API_KEY，请在环境变量中设置'});
                return
